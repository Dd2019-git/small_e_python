...
def get_single_articleFromSQL(F_Connectors,F_CurrentMode, F_ArticleTable, F_ArticleID, F_LogFile):
...
# Сначала надо выкинуть все, что не входят в список разрешенных чатов. 
    ...
    # Начинаю доработку логики. Будет использоваться две функции (потом больше - после чисток)
    # функция 1 - получение единичного блока данных из SQL (оно там приезжает как dict, 
    # так что надо подумать над правильной обработкой)
    Table4Test1 = "generalquestions"
    ID4Test1 = "1"
    get_single_articleFromSQL(PGConn1, CurrentMode, Table4Test1, ID4Test1, MyGlobalReqLog)
    # вот получили (1, 'Bot INFO', 'Bot version 001', ['#infotesttag', '#infotag2'], 
    # ['4 _4link2longstory'], ['5 _5link2textfield5_legacy'], 6, ['7 reserv1 geogoogle'], '8_reserv2', ' 9_reserv3')
